 ...
 * <p>
 * The Vanilla sign GUI does not use {} and thus
...
public class SignGui implements GuiInterface {
    ...
    public SignGui(ServerPlayer player)  {
        this.player = player;
        this.signEntity = new VirtualSignBlockEntity(new BlockPos(player.getBlockX(), Math.min(player.level().getMaxBuildHeight() - 1, player.getBlockY() + 5), player.getBlockZ()), Blocks.OAK_SIGN.defaultBlockState());
    }
    ...
    public boolean open() {
        ...
        this.player.connection.send(Objects.requireNonNull(this.signEntity.getUpdatePacket()));
    ...
    public void close(boolean alreadyClosed) {
        if (this.open && !this.reOpen) {
            ...
            this.player.connection.send(new ClientboundBlockUpdatePacket(signEntity.getBlockPos(), signEntity.getBlockState()));
